...
class AirQualityDatabase:
    ...
    def _create_collections(self):
        ...
        self.db.model_performance.create_index([('model_name', ASCENDING), ('timestamp', DESCENDING)])
        ...
        try:
            ...
            for record in records:
                doc = {
                    ...
                    'temperature': float(record.get('temperature', 0)) if pd.notna(record.get('temperature')) else None,
                    'humidity': float(record.get('humidity', 0)) if pd.notna(record.get('humidity')) else None,
                ...
                for col in record.keys():
                    if col not in doc and col not in ['index', 'Date']:
                        if pd.notna(record[col]):
                            doc[col] = float(record[col]) if isinstance(record[col], (int, float)) else record[col]
    ...
    def insert_live_data(self, data: Dict):
        ...
        try:
            document = {
                ...
                'temperature': float(data.get('temperature', 0)) if data.get('temperature') is not None else None,
                'humidity': float(data.get('humidity', 0)) if data.get('humidity') is not None else None,
                'pressure': float(data.get('pressure', 0)) if data.get('pressure') is not None else None,
                'wind_speed': float(data.get('wind_speed', 0)) if data.get('wind_speed') is not None else None,
                'wind_direction': float(data.get('wind_direction', 0)) if data.get('wind_direction') is not None else None,
    ...
    def insert_prediction(self, prediction_timestamp: datetime, target_timestamp: datetime,
                         predicted_aqi: float, model_name: str, predicted_category: str,
    ...
    def get_recent_data(self, hours: int = 24, table: str = 'live_data') -> pd.DataFrame:
    ...
    def get_data_statistics(self) -> Dict:
        ...
        try:
            stats = {
                ...
                'predictions': {
                    'count': self.db.predictions.count_documents({}),
                    'latest': None
                }
            }
    ...
    def cleanup_old_data(self, keep_days: int = 90):
        ...
        try:
            ...
            result_hist = self.db.historical_data.delete_many(
                {'timestamp': {'$lt': cutoff_date}}
            )
            
            # Cleanup live data
            result_live = self.db.live_data.delete_many(
                {'timestamp': {'$lt': cutoff_date}}
            )
            ...
            print(f"✅ Cleaned up {result_hist.deleted_count} historical, "
